...
# 우리측 식별자(공인 IP 또는 FQDN) #CGW 공인 IP값 넣기
LEFTID_FQDN_OR_IP="3.0.125.86"

# 온프레미스 광고 대역
ADVERTISE_CIDR="10.4.0.0/16"

# 상대(AWS VPC)
AWS_VPC_CIDR="10.3.0.0/16"

# (선택) 내부 정적 라우트 예시 — 필요 시 조정
IDC_STATIC_SUBNET="10.4.1.0/24"
IDC_LAN_GW="10.4.1.1"
...
firewall-cmd --permanent --policy vpn-to-lan \
  ...
  --add-rich-rule="rule family=ipv4 source address=10.1.0.0/16 destination address=${ADVERTISE_CIDR} accept" \
  --add-rich-rule="rule family=ipv4 source address=10.2.0.0/16 destination address=${ADVERTISE_CIDR} accept"
...
firewall-cmd --permanent --policy lan-to-vpn \
  ...
  --add-rich-rule="rule family=ipv4 source address=${ADVERTISE_CIDR} destination address=10.1.0.0/16 accept" \
  --add-rich-rule="rule family=ipv4 source address=${ADVERTISE_CIDR} destination address=10.2.0.0/16 accept"
